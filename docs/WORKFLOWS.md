...
| `work-unit-implementation` | `wefter work-unit run` | `/wefter-run-work-unit` | Partial |

The first implementation release keeps `documentation-audit` executable and registers the full architecture. `work-unit-implementation` can generate planning runs; deterministic guards and OpenCode agents are next.
