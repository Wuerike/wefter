...
| `work-unit-implementation` | `wefter work-unit run`, `wefter work-unit guard` | `/wefter-run-work-unit` | Partial |

The first implementation release keeps `documentation-audit` executable and registers the full architecture. `work-unit-implementation` can generate planning runs and enforce deterministic task/review guards; OpenCode agents are next.
